...
function User() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "Hiba történt a felhasználó adatainak betöltésekor:",
        error
      );
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const getHistoryCars = async () => {
      ...
      const response = await fetch(
        `https://localhost:7175/api/Rents/user-rents/${userId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        ...
        console.error(
          "Nem sikerült lekérni az autók történetét:",
          response.status
        );
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const addressResponse = await fetch(
        "https://localhost:7175/api/Users/update-address",
        {
          method: "PUT",
          headers: {
      ...
      const phoneResponse = await fetch(
        "https://localhost:7175/api/Users/update-phone",
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...
            <div className="bg-blue-500 shadow-light overflow-y-scroll h-48 w-auto rounded-lg">
              ...
              {historyCars.length > 0 ? (
                historyCars.map((rent) => (
                  <div
                    className="m-5 p-5 border-2 rounded-lg group relative"
                    key={rent.id}
                  >
                    {/* Tooltip */}
        ...
        >
          ...
          <div className="flex-col flex space-y-2">
            {editing ? (
              ...
              <>
                <div>Telefonszám: {user.phone}</div>
              </>
            )}
            {editing ? (
              <input
